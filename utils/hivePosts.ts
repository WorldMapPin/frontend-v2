  ...
  try {
    ...
    const cleanImageUrl = (url: string): string | null => {
      ...
      if (url.includes("](")) {
        ...
        if (
          beforeBracket.startsWith("http://") ||
          beforeBracket.startsWith("https://")
        ) {
    ...
    const extractFirstImageFromBody = (body: string): string | null => {
      ...
      const standaloneMatch = body.match(
      ...
      const leopediaMatch = body.match(
        /^\s*(https?:\/\/img\.leopedia\.io\/[^\s]+)\s*$/im,
      );
    ...
    if (!coverImage && metadata.original_author && metadata.original_permlink) {
      try {
        ...
        if (originalResponse.ok) {
          ...
          if (originalData.post) {
            ...
            const originalImageArray =
              originalMetadata.image || originalMetadata.images;
            if (
              originalImageArray &&
              Array.isArray(originalImageArray) &&
              originalImageArray.length > 0
            ) {
            ...
            if (!coverImage && originalPost.body) {
              const originalBodyImage = extractFirstImageFromBody(
                originalPost.body,
              );
        ...
        console.warn(
          `Error fetching original post cover image for crosspost @${metadata.original_author}/${metadata.original_permlink}:`,
          error,
        );
      }
    }
    ...
    const isMuted =
      post.title === "error" && (post.body || "").trim() === "error";
...
export async function fetchPostWithRetry(
  ...
  for (let attempt = 0; attempt < maxRetries; attempt++) {
    try {
      ...
      lastError = error;
      console.log(
        `Retry ${attempt + 1}/${maxRetries} for ${author}/${permlink}`,
      );
  ...
  console.error(
    `Failed to fetch post after ${maxRetries} attempts:`,
    lastError,
  );
  return null;
}
...
function processBridgePost(post: any): ProcessedPost | null {
  ...
  if (Array.isArray(imageArray)) {
    for (const img of imageArray) {
      ...
      if (cleaned) {
        coverImage = optimizeImageUrl(cleaned, "thumb");
        break;
      }
    }
  }
...
export async function batchFetchPosts(
  ...
  try {
    ...
    for (let j = 0; j < batchResults.length; j++) {
      const processed = batchResults[j]
        ? processBridgePost(batchResults[j])
        : null;
