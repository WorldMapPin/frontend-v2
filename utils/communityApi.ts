...
import {
  convertDatafromApitoGeojson,
  convertDistriatorBusinessesToGeojson,
} from "./dataConversion";
import {
  fetchAllDistriatorBusinesses,
  DistriatorFetchResult,
} from "./distriatorApi";
...
export async function fetchCommunityPins(community: Community): Promise<{
  ...
  try {
    ...
    if (community.id === "spendhbd") {
      ...
      console.log(
        `Fetched ${result.allStores.length} total stores: ${result.mappedStores.length} mapped, ${result.unmappedStores.length} unmapped`,
      );
      ...
      if (result.unmappedStores.length > 0) {
        console.log(
          "Unmapped stores (no valid coordinates):",
          result.unmappedStores.map((s) => s.profile.displayName),
        );
      }

      // Convert only mapped stores to GeoJSON format
      const geoJsonData = await convertDistriatorBusinessesToGeojson(
        result.mappedStores,
        result.reviewCounts,
        result.storeReviews,
      );
      console.log(
        `GeoJSON data converted for ${community.name}:`,
    ...
    if (community.id === "foodie") {
      ...
      console.log(
        `Fetched ${response.data?.length || 0} food-related pins for ${community.name}`,
      );

      // Convert to GeoJSON format
      const geoJsonData = await convertDatafromApitoGeojson(
        response.data || [],
      );
      console.log(
        `GeoJSON data converted for ${community.name}:`,
    ...
    if (community.isDefault) {
      ...
      throw new Error(
        `No API endpoint or tag configured for community: ${community.name}`,
      );
    }

    console.log(
      `API response received for ${community.name}:`,
    ...
    console.error(
      `Error fetching pins for community ${community.name}:`,
      error,
    );
    throw error;
  }
