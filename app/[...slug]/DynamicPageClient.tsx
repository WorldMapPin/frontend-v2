...
import { UserProfile } from "@/app/user/[username]/UserProfile";
 ...
 * - /@username (profile page) - 1 segment starting with @
 * - /@author/permlink (post page) - 2 segments, first starts with @
 */
export default function DynamicPageClient() {
  ...
  // Determine route type based on slug structure
  const startsWithAt = decodedFirst.startsWith("@");
  const segmentCount = slugArray?.length || 0;

  // Route types:
  // - /@username (1 segment) = profile
  // - /@author/permlink (2 segments) = post
  // - anything else = 404
  const isProfileRoute = startsWithAt && segmentCount === 1;
  const isPostRoute = startsWithAt && segmentCount >= 2;

  // If doesn't start with @, show 404
  if (!startsWithAt) {
    notFound();
  }

  // Render profile or post based on route type
  if (isProfileRoute) {
...
// Post page component (extracted from original)
function PostPage({ author, permlink }: { author: string; permlink: string }) {
    ...
    try {
      ...
      cleanHtml = cleanHtml.replace(
        ...
        (match, before, src, after) => {
          ...
          const optimizedSrc = optimizeImageForPost(
            src,
            IMAGE_SIZES.postContent.width,
            IMAGE_SIZES.postContent.height,
          );
            ...
            >
              ...
              <div className="animate-pulse">
                ...
                <div className="p-4 sm:p-6 lg:p-8">
                  <div
                    className="h-6 sm:h-8 rounded w-3/4 mb-3 sm:mb-4"
                    style={{ backgroundColor: "var(--skeleton-bg)" }}
                  ></div>
                  <div
                    className="h-4 rounded w-1/2 mb-4 sm:mb-6"
                    style={{ backgroundColor: "var(--skeleton-bg)" }}
                  ></div>
      ...
      >
        <div className="relative z-10">
          <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 sm:py-12">
            <div
              className="rounded-lg shadow-md p-4 sm:p-6 lg:p-8 text-center"
              style={{ backgroundColor: "var(--card-bg)" }}
            >
              <svg
              ...
              >
                This post has been muted by community moderators and its content
  ...
  const highResCoverImage = post?.coverImage 
    ? post.coverImage.replace("/150x0/", "/1200x0/").replace("/256x512/", "/1200x0/")
    : null;
          ...
          <article>
            ...
            {post.coverImage && (
              <div className="mb-4 sm:mb-6 lg:mb-8">
                ...
                >
                  <ProgressiveImage
                    src={highResCoverImage as string}
                    alt={post.title}
                    fill
                    mode="post"
                    placeholder="blur"
            ...
            <div className="p-4 sm:p-6 lg:p-8">
              ...
              <div className="mb-3 sm:mb-4">
                <div className="flex sm:hidden items-center justify-between gap-1.5">
                  <div className="flex items-center gap-1.5 min-w-0 flex-1">
                    ...
                    <Link href={`/@${post.author}`} className="flex-shrink-0">
                      <img
                        src={`https://images.ecency.com/u/${post.author}/avatar`}
                  ...
                  <div className="flex items-center gap-1 flex-shrink-0">
                    ...
                    >
                      <svg
                        className="w-3 h-3 flex-shrink-0 read-page-like-icon"
                        fill="none"
                        viewBox="0 0 24 24"
                        stroke="currentColor"
                      ...
                      <span
                        className="font-medium text-xs read-page-like-text"
                        style={{ color: "var(--like-color)" }}
                      >
                        {post.votes}
                      </span>
                    ...
                    >
                      <svg
                        className="w-3 h-3 flex-shrink-0 read-page-comment-icon"
                        fill="none"
                        viewBox="0 0 24 24"
                        stroke="currentColor"
                      ...
                      <span
                        className="font-medium text-xs read-page-comment-text"
                        style={{ color: "var(--comment-color)" }}
                      >
                        {post.comments}
                      </span>
                ...
                <div className="hidden sm:flex items-center justify-between gap-2">
                  ...
                  <div className="flex items-center gap-1.5">
                    ...
                    >
                      <svg
                        className="w-4 h-4 read-page-like-icon"
                        fill="none"
                        viewBox="0 0 24 24"
                        stroke="currentColor"
                        style={{ color: "var(--like-color)" }}
                      ...
                      <span
                        className="font-medium text-sm read-page-like-text"
                        style={{ color: "var(--like-color)" }}
                      >
                        {post.votes}
                      </span>
                    ...
                    >
                      <svg
                        className="w-4 h-4 read-page-comment-icon"
                        fill="none"
                        viewBox="0 0 24 24"
                        stroke="currentColor"
                        style={{ color: "var(--comment-color)" }}
                      ...
                      <span
                        className="font-medium text-sm read-page-comment-text"
                        style={{ color: "var(--comment-color)" }}
                      >
                        {post.comments}
                      </span>
              ...
              {post.tags.length > 0 && (
                <div className="mb-3 sm:mb-4">
                  <div className="flex flex-wrap items-center gap-1.5 sm:gap-2">
                    {(showAllTags ? post.tags : post.tags.slice(0, 4)).map(
                      (tag, index) => (
                        <span
                          key={index}
              ...
              <div className="mt-8 sm:mt-10 lg:mt-12 flex justify-center">
                ...
                >
                  <span
                    className="font-semibold text-sm sm:text-base lg:text-lg read-page-open-platform-text"
                    style={{ color: "#FFFFFF" }}
                  >
                    Open on{" "}
                    {post.canonicalUrl.includes("inleo.io")
                      ? "InLeo"
                      : post.canonicalUrl.includes("ecency.com")
                        ? "Ecency"
                        : post.canonicalUrl.includes("hive.blog")
                          ? "Hive.blog"
                          : "PeakD"}
                  </span>
                  <svg
                    className="w-5 h-5 sm:w-6 sm:h-6 read-page-open-platform-icon"
                    fill="none"
                    viewBox="0 0 24 24"
                    stroke="currentColor"
                    style={{ color: "#FFFFFF" }}
