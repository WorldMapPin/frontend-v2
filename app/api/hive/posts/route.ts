...
function setCached(key: string, data: PostsResponse) {
  if (postCache.size >= MAX_CACHE_ENTRIES) {
    const oldest = [...postCache.entries()].sort(
...
const originalCoverCache = new Map<
  string,
  { cover: string | null; timestamp: number }
>();
const ORIGINAL_COVER_TTL = 5 * 60_000;

async function fetchOriginalCoverImage(
  author: string,
  permlink: string,
): Promise<string | null> {
  const key = `${author}/${permlink}`;
  const cached = originalCoverCache.get(key);
  if (cached && Date.now() - cached.timestamp < ORIGINAL_COVER_TTL)
    return cached.cover;
  ...
  try {
    ...
    console.warn(
      `fetchOriginalCoverImage failed for @${author}/${permlink}`,
      error,
    );
    originalCoverCache.set(key, { cover: null, timestamp: Date.now() });
    return null;
...
async function processRankedPost(post: any): Promise<ProcessedPost> {
  ...
  if (!coverImage && metadata?.original_author && metadata?.original_permlink) {
    coverImage = await fetchOriginalCoverImage(
      metadata.original_author,
      metadata.original_permlink,
    );
  }
...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    if (!Array.isArray(rawPosts)) {
      return NextResponse.json(
        { error: "Unexpected response from Hive API" },
        { status: 502 },
      );
    }
    ...
    console.log(
      `hive/posts: cache MISS — ${rawPosts.length} raw, ${processed.length} visible in ${Date.now() - t0}ms`,
    );
