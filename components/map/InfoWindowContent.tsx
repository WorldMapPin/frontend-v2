...
function SkeletonCard({ index }: { index: number }) {
  return (
    ...
    >
      ...
      <div
        className="absolute left-3 sm:left-4 flex flex-row gap-1.5 sm:gap-2 z-20 top-[180px] sm:top-[220px] lg:top-[249.6px]"
      ...
      {/* Date skeleton */}
      <div
        className="absolute right-3 sm:right-4 z-20 top-[180px] sm:top-[220px] lg:top-[249.6px]"
        style={{ transform: "translateY(-50%)" }}
      >
...
export const InfoWindowContent = memo(
  ({
      ...
      try {
        ...
        const postsWithCoordinates = await Promise.all(
          features.map(async (feature) => {
            try {
              ...
              console.error(
                `Error fetching post for feature ${feature.id}:`,
                err,
              );
              return null;
            }
        ...
        await fetchPostsProgressive(
          ...
          (newPosts) => {
            ...
            setLoadingProgress(
              Math.round((allPosts.length / curatedPosts.length) * 100),
            );
          },
          10, // Concurrency
        );
    ...
    if (loading && selectedFeatures.length === 0) {
      ...
      return (
        <div className="space-y-3 sm:space-y-4">
          {/* Loading Header */}
          <div
            className="backdrop-blur-sm border-b pb-3 sm:pb-4 lg:hidden sticky top-0 z-10"
            style={{
              backgroundColor: "var(--card-bg)",
    ...
    if (error) {
      return (
        <div
          className="text-center py-8 sm:py-12 px-4 rounded-xl border"
          style={{
        ...
        >
          ...
          <p
            className="text-xs sm:text-sm mb-4 sm:mb-6 max-w-md mx-auto"
    ...
    if (selectedFeatures.length > 0) {
      return (
        <div className="space-y-3 sm:space-y-6">
          ...
          {!hideHeader && (
            <div
              className="backdrop-blur-sm border-b pb-3 sm:pb-5 mb-1 sm:mb-2"
              ...
              <div className="flex items-center justify-between gap-2">
                <div className="flex items-center gap-2 sm:gap-3">
                  ...
                  <div className="min-w-0">
                    <h2
                      className="text-base sm:text-xl lg:text-2xl font-bold leading-tight truncate"
                      style={{
                        fontFamily: "var(--font-lexend)",
                        color: "var(--text-primary)",
                      }}
